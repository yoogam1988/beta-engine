  ...
  <Col>
    ...
    <Properties>
      <Property name='message_id' type='str' key='message_id'>
        For text messages generated by AtomFlow, simply pass the generated message-id directly. The backend will use the message-id to look up the corresponding content and synthesize the voice information directly. If both message_id and text are provided simultaneously, the message_id is given priority.
      </Property>
...
--header 'Authorization: Bearer ENTER-YOUR-SECRET-KEY' \\
--form 'text=Hello AtomFlow;user=abc-123;message_id=5ad4cb98-f0c7-4085-b384-88c403be6290`}
    />
...
-H 'Authorization: Bearer {api_key}'`}
    ...
    {
      "tool_icons": {
        "dalle2": "/help/console/api/workspaces/current/tool-provider/builtin/dalle/icon",
        "api_tool": {
          "background": "#252525",
          "content": "\ud83d\ude01"
        }
...
--header 'Authorization: Bearer {api_key}' \\
--header 'Content-Type: application/json' \\
--data-raw '{"question": "What is your name?","answer": "I am AtomFlow."}'`}
    />
